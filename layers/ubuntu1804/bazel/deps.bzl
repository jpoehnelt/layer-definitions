...
def deps():
    ...
    if "ubuntu1804_bazel_installer" not in excludes:
        http_file(
            name = "ubuntu1804_bazel_installer",
    ...
    if "ubuntu1804_bazel_debs" not in excludes:
        http_file(
            name = "ubuntu1804_bazel_debs",
            downloaded_file_path = DEBS_TARBALL.revision + "_bazel_debs.tar",
            sha256 = DEBS_TARBALL.sha256,
            urls = [
                "https://storage.googleapis.com/layer-deps/ubuntu1804/bazel/debs/" + DEBS_TARBALL.revision + "_bazel_debs.tar",
            ],
        )
